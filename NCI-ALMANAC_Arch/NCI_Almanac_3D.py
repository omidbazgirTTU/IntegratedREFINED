...
SAVE_PATH = "/home/obazgir/REFINED/Volumetric_REFINED/NCI_Almanac/Vol_Res/"
...
for i in range(20):
    ...
    import pickle
    import math
    with open('/home/obazgir/REFINED/Volumetric_REFINED/theMapping_Init_LE.pickle','rb') as file:
        gene_names_LE,coords_LE,map_in_int_LE = pickle.load(file)
    # LLE
    with open('/home/obazgir/REFINED/Volumetric_REFINED/theMapping_Init_LLE.pickle','rb') as file:
        gene_names_LLE,coords_LLE,map_in_int_LLE = pickle.load(file)
    # ISOMAP
    with open('/home/obazgir/REFINED/Volumetric_REFINED/theMapping_Init_ISO.pickle','rb') as file:
        gene_names_ISO,coords_ISO,map_in_int_ISO = pickle.load(file)
    # MDS
    with open('/home/obazgir/REFINED/Volumetric_REFINED/theMapping_REFINED_5.pickle','rb') as file:
    ...
    def CNN_model(Width,Height):
        ...
        x1 = layers.Conv3D(36, (nb_conv, nb_conv,4),padding='valid',strides=2,dilation_rate=1)(input1)
        x1 = layers.BatchNormalization()(x1)
        x1 = layers.Activation('relu')(x1)
        x1 = layers.Conv3D(218, (nb_conv, nb_conv,1),padding='valid',strides=2,dilation_rate=1)(x1)
        x1 = layers.BatchNormalization()(x1)
        x1 = layers.Activation('relu')(x1)
        ...
        y1 = layers.Conv3D(36, (nb_conv, nb_conv,4),padding='valid',strides=2,dilation_rate=1)(input2)
        y1 = layers.BatchNormalization()(y1)
        y1 = layers.Activation('relu')(y1)
        y1 = layers.Conv3D(218, (nb_conv, nb_conv,1),padding='valid',strides=2,dilation_rate=1)(y1)
        y1 = layers.BatchNormalization()(y1)
        y1 = layers.Activation('relu')(y1)
        ...
        initial_learning_rate = 0.0008087154906596161
        lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
            initial_learning_rate,
            decay_steps=191265,
            decay_rate= 0.5945683057633432,
            staircase=True)
    ...
    Y_Val_Save_PD = pd.DataFrame(data = Y_Val_Save , columns = ["Y_Val","Y_3D"])
    Y_Test_Save_PD = pd.DataFrame(data = Y_Test_Save , columns = ["Y_Test","Y_3D"])
    # Name correction
    Cell_Name = Cells[i] 
    if Cell_Name.find("/"):
