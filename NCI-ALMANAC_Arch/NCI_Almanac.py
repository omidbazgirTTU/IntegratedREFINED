...
SAVE_PATH = "C:\\Users\\obazgir\\Dropbox\\Stacking REFINED\\Scripts\\NCI-Almanac\\4Init\\"
...
NCI_ALM_PD = pd.read_csv("C:\\Users\\obazgir\\Desktop\\REFINED project\\NCI-ALMANAC\\ComboDrugGrowth_Nov2017\\ComboDrugGrowth_Nov2017.csv")
...
for i in range(20):
    ...
    Y_Val_Save = np.zeros(((len(Y_Validation)),5))
    Y_Val_Save[:,0] = Y_Validation.reshape(-1)
    Y_Test_Save = np.zeros(((len(Y_Test)),5))
    ...
    for mdl in MODELS:
        if mdl == "LE":
            ...
            X_Val_D1_REFINED = REFINED_Im_Gen(X_Val_D1,nn, map_in_int_LE, gene_names_LE,coords_LE)
            X_Test_D1_REFINED = REFINED_Im_Gen(X_Test_D1,nn, map_in_int_LE, gene_names_LE,coords_LE)
            
            X_Train_D2_REFINED = REFINED_Im_Gen(X_Train_D2,nn, map_in_int_LE, gene_names_LE,coords_LE)
            X_Val_D2_REFINED = REFINED_Im_Gen(X_Val_D2,nn, map_in_int_LE, gene_names_LE,coords_LE)
            X_Test_D2_REFINED = REFINED_Im_Gen(X_Test_D2,nn,map_in_int_LE, gene_names_LE,coords_LE)
        
        elif mdl == "MDS":
            
            X_Train_D1_REFINED = REFINED_Im_Gen(X_Train_D1,nn, map_in_int_MDS, gene_names_MDS,coords_MDS)
            X_Val_D1_REFINED = REFINED_Im_Gen(X_Val_D1,nn, map_in_int_MDS, gene_names_MDS,coords_MDS)
            X_Test_D1_REFINED = REFINED_Im_Gen(X_Test_D1,nn, map_in_int_MDS, gene_names_MDS,coords_MDS)
            
            X_Train_D2_REFINED = REFINED_Im_Gen(X_Train_D2,nn, map_in_int_MDS, gene_names_MDS,coords_MDS)
            X_Val_D2_REFINED = REFINED_Im_Gen(X_Val_D2,nn, map_in_int_MDS, gene_names_MDS,coords_MDS)
            X_Test_D2_REFINED = REFINED_Im_Gen(X_Test_D2,nn, map_in_int_MDS, gene_names_MDS,coords_MDS)
            
            
        elif mdl == "LLE":
            X_Train_D1_REFINED = REFINED_Im_Gen(X_Train_D1,nn, map_in_int_LLE, gene_names_LLE,coords_LLE)
            X_Val_D1_REFINED = REFINED_Im_Gen(X_Val_D1,nn, map_in_int_LLE, gene_names_LLE,coords_LLE)
            X_Test_D1_REFINED = REFINED_Im_Gen(X_Test_D1,nn, map_in_int_LLE, gene_names_LLE,coords_LLE)
            
            X_Train_D2_REFINED = REFINED_Im_Gen(X_Train_D2,nn, map_in_int_LLE, gene_names_LLE,coords_LLE)
            X_Val_D2_REFINED = REFINED_Im_Gen(X_Val_D2,nn, map_in_int_LLE, gene_names_LLE,coords_LLE)
            X_Test_D2_REFINED = REFINED_Im_Gen(X_Test_D2,nn,map_in_int_LLE, gene_names_LLE,coords_LLE)
            
        elif mdl == "ISO":
            X_Train_D1_REFINED = REFINED_Im_Gen(X_Train_D1,nn, map_in_int_ISO, gene_names_ISO,coords_ISO)
        ...
        def CNN_model(Width,Height):
            ...
            x1 = layers.Conv2D(41, (nb_conv, nb_conv),padding='valid',strides=2,dilation_rate=1)(input1)
            x1 = layers.BatchNormalization()(x1)
            x1 = layers.Activation('relu')(x1)
            x1 = layers.Conv2D(87, (nb_conv, nb_conv),padding='valid',strides=2,dilation_rate=1)(x1)
            x1 = layers.BatchNormalization()(x1)
            x1 = layers.Activation('relu')(x1)
            ...
            y1 = layers.Conv2D(41, (nb_conv, nb_conv),padding='valid',strides=2,dilation_rate=1)(input2)
            y1 = layers.BatchNormalization()(y1)
            y1 = layers.Activation('relu')(y1)
            y1 = layers.Conv2D(87, (nb_conv, nb_conv),padding='valid',strides=2,dilation_rate=1)(y1)
            y1 = layers.BatchNormalization()(y1)
            y1 = layers.Activation('relu')(y1)
            ...
            initial_learning_rate = 0.0008054081579805455
            lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
                initial_learning_rate,
                decay_steps=228784,
                decay_rate=0.42397284845443234,
                staircase=True)
    ...
    Y_Val_Save_PD = pd.DataFrame(data = Y_Val_Save , columns = ["Y_Val","LE","LLE","MDS","ISO"])
    Y_Test_Save_PD = pd.DataFrame(data = Y_Test_Save , columns = ["Y_Test","LE","LLE","MDS","ISO"])
    # Name correction
    Cell_Name = Cells[i] 
    if Cell_Name.find("/"):
