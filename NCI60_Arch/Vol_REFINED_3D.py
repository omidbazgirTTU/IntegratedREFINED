...
#cell_lines = ["HCC_2998"]
Results_Dic = {}
SAVE_PATH = "/home/obazgir/REFINED/Volumetric_REFINED/NCI_Regression/"
#%%
for SEL_CEL in cell_lines:
    ...
    # LE
    import math
    with open('theMapping_Init_LE.pickle','rb') as file:
        gene_names_LE,coords_LE,map_in_int_LE = pickle.load(file)
    # LLE
    with open('theMapping_Init_LLE.pickle','rb') as file:
        gene_names_LLE,coords_LLE,map_in_int_LLE = pickle.load(file)
    # ISOMAP
    with open('theMapping_Init_ISO.pickle','rb') as file:
        gene_names_ISO,coords_ISO,map_in_int_ISO = pickle.load(file)
    # MDS
    with open('theMapping_REFINED_5.pickle','rb') as file:
    ...
    for modell in Model_Names:
        ...
        def CNN_model(Width,Height,):
            ...
            model.add(layers.Conv3D(48, kernel_size = (nb_conv, nb_conv,4),padding='valid',strides=(2,2,1),dilation_rate=1,input_shape=(Width, Height,4,1)))
            model.add(layers.BatchNormalization())
            model.add(layers.Activation('relu'))
            model.add(layers.Conv3D(128, kernel_size = (nb_conv, nb_conv,1),padding='valid',strides=(2,2,1),dilation_rate=1))
            model.add(layers.BatchNormalization())
            model.add(layers.Activation('relu'))
